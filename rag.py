...
def classify_form_type(question: str) -> str:
    """서식 종류를 세분화하여 분류 (구체적 키워드를 먼저 매칭)"""
    q = question.lower()
    # 금융거래정보제출명령 — "작성/써줘" 맥락에서만 매칭 (정보 제공 시 오분류 방지)
    if any(kw in q for kw in ["금융거래정보", "제출명령"]):
        return "financial_info"
    if any(kw in q for kw in ["사실조회", "신원확인", "통신사 조회"]):
        return "fact_inquiry"
    if any(kw in q for kw in ["내용증명", "통고", "최고"]):
        return "certified_mail"
    # 고소장/고발장을 소장보다 먼저 체크 ("고소장"에 "소장"이 포함되므로)
    if any(kw in q for kw in ["고소장", "고발장"]):
        return "accusation"
    if any(kw in q for kw in ["소장", "민사소장", "민사", "손해배상"]):
        return "civil"
    if any(kw in q for kw in ["형사", "고소"]):
        return "accusation"
    return "unknown"
...
1. 민사소송 루트 (사기꾼에게 법적 책임을 묻고 배상받기)
   - 법원에 소장 제출 (피고: 성명불상)
   - 사실조회/금융거래정보제출명령으로 사기꾼 신원 특정
   - 판결 → 강제집행
...
9. 컨텍스트에 '실무 지식' 또는 '실제 소송 경험'이 포함되어 있으면, 이를 우선 참고하여 현실적인 답변을 제공하세요. 이론적 절차뿐 아니라 실제로 겪을 수 있는 어려움(보정명령 반복, 농협 계좌 구분, 미성년자 문제 등)도 함께 안내하세요.
10. [필수] 법률 용어가 처음 등장할 때 반드시 괄호 안에 일상 언어로 풀어 설명하세요. 이 규칙은 모든 답변에 예외 없이 적용됩니다. 사용자는 법률 지식이 전혀 없는 일반인입니다.
예시:
- 소장(법원에 내는 피해 신고서) / 소취하(소송을 중간에 그만두는 것)
- 성명불상(이름을 모르는 상태) / 보정명령(법원이 서류를 고쳐서 다시 내라는 통보)
- 강제집행(법원 힘으로 강제로 돈을 받아내는 것) / 가집행(판결 확정 전에도 미리 돈을 받을 수 있는 것)
- 사실조회(법원이 은행 등에 정보를 요청하는 것) / 금융거래정보제출명령(법원이 은행에 계좌 주인 정보를 내놓으라고 명령하는 것)
- 인지대(법원 수수료) / 송달료(서류 우편 비용) / 기소(검찰이 재판에 넘기는 것)
11. 답변 마지막에 "이 내용은 일반적인 법률 정보이며, 구체적인 사안에 대해서는 변호사와 상담하시는 것을 권장합니다."라는 면책 안내를 포함하세요.
...
아래 JSON 템플릿을 **반드시 그대로** 따라서 법률 서식 초안을 작성해 주세요.

**절대 규칙 (반드시 지키세요):**
1. 템플릿의 각 section을 순서대로 출력하세요. section을 빼거나 순서를 바꾸지 마세요.
2. 각 section의 title과 template_text를 그대로 사용하세요. 임의로 제목을 바꾸거나 내용을 요약하지 마세요.
3. template_text에서 [placeholder] 부분만 사용자 정보로 교체하세요. 나머지 문구는 한 글자도 바꾸지 마세요.
4. 사용자가 제공한 날짜, 금액, 이름을 절대 변경하지 마세요.
5. 소장에는 사건번호를 넣지 마세요 (접수 후 부여됨). 사건번호는 사실조회/금융거래정보제출명령에만 필요합니다.

**작성 흐름:**
1. 아직 제공되지 않은 required 필드가 있으면 → 필요한 정보 목록을 사용자에게 요청하세요
2. 모든 required 필드가 제공되면 → 템플릿 구조대로 완성된 서식을 출력하세요
3. 서식 출력이 끝나면 반드시 "---" 구분선 아래에 다음을 추가하세요:
   - **안내사항**: 템플릿 JSON의 "notes" 배열에 있는 항목을 번호 매겨 모두 출력
   - **제출 방법**: 템플릿 JSON의 "submit_info" 객체의 내용을 모두 출력
   이 두 항목을 빠뜨리면 안 됩니다. 서식 본문만 출력하고 끝내지 마세요.
